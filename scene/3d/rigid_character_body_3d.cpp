#include "rigid_character_body_3d.h"
...
bool  RigidCharacterBody3D::move_and_slide() {
...
void RigidCharacterBody3D::check_on_floor() {
    double delta = Engine::get_singleton()->is_in_physics_frame() ? get_physics_process_delta_time() : get_process_delta_time();
    ...
    PhysicsServer3D::MotionResult floor_result;
    if (move_and_collide(parameters, floor_result, true, false)) {
        // motion_results.push_back(floor_result);
...
void  RigidCharacterBody3D::_move_and_slide_grounded(double p_delta, bool p_was_on_floor) {
...
void  RigidCharacterBody3D::_move_and_slide_floating(double p_delta) {
	Vector3 motion = velocity * p_delta;
...
void  RigidCharacterBody3D::apply_floor_snap() {
	if (collision_state.floor) {
		return;
	}
...
void  RigidCharacterBody3D::_snap_on_floor(bool p_was_on_floor, bool p_vel_dir_facing_up) {
...
bool  RigidCharacterBody3D::_on_floor_if_snapped(bool p_was_on_floor, bool p_vel_dir_facing_up) {
...
void  RigidCharacterBody3D::_set_collision_direction(const PhysicsServer3D::MotionResult &p_result, CollisionState &r_state, CollisionState p_apply_state) {
	r_state.state = 0;
...
void  RigidCharacterBody3D::_set_platform_data(const PhysicsServer3D::MotionCollision &p_collision) {
...
void  RigidCharacterBody3D::set_safe_margin(real_t p_margin) {
	margin = p_margin;
}

real_t  RigidCharacterBody3D::get_safe_margin() const {
	return margin;
}

const Vector3 &RigidCharacterBody3D::get_velocity() const {
	return velocity;
}

void  RigidCharacterBody3D::set_velocity(const Vector3 &p_velocity) {
	velocity = p_velocity;
}

bool  RigidCharacterBody3D::is_on_floor() const {
	return collision_state.floor;
}

bool  RigidCharacterBody3D::is_on_floor_only() const {
	return collision_state.floor && !collision_state.wall && !collision_state.ceiling;
}

bool  RigidCharacterBody3D::is_on_wall() const {
	return collision_state.wall;
}

bool  RigidCharacterBody3D::is_on_wall_only() const {
	return collision_state.wall && !collision_state.floor && !collision_state.ceiling;
}

bool  RigidCharacterBody3D::is_on_ceiling() const {
	return collision_state.ceiling;
}

bool  RigidCharacterBody3D::is_on_ceiling_only() const {
	return collision_state.ceiling && !collision_state.floor && !collision_state.wall;
}

const Vector3 &RigidCharacterBody3D::get_floor_normal() const {
	return floor_normal;
}

const Vector3 &RigidCharacterBody3D::get_wall_normal() const {
	return wall_normal;
}

const Vector3 &RigidCharacterBody3D::get_last_motion() const {
	return last_motion;
}

Vector3  RigidCharacterBody3D::get_position_delta() const {
	return get_global_transform().origin - previous_position;
}

const Vector3 &RigidCharacterBody3D::get_real_velocity() const {
	return real_velocity;
}

real_t  RigidCharacterBody3D::get_floor_angle(const Vector3 &p_up_direction) const {
	ERR_FAIL_COND_V(p_up_direction == Vector3(), 0);
	return Math::acos(floor_normal.dot(p_up_direction));
}

const Vector3 &RigidCharacterBody3D::get_platform_velocity() const {
	return platform_velocity;
}

const Vector3 &RigidCharacterBody3D::get_platform_angular_velocity() const {
	return platform_angular_velocity;
}

Vector3  RigidCharacterBody3D::get_linear_velocity() const {
	return get_real_velocity();
}

int  RigidCharacterBody3D::get_slide_collision_count() const {
	return motion_results.size();
}

PhysicsServer3D::MotionResult  RigidCharacterBody3D::get_slide_collision(int p_bounce) const {
	ERR_FAIL_INDEX_V(p_bounce, motion_results.size(), PhysicsServer3D::MotionResult());
	return motion_results[p_bounce];
}

// Ref<KinematicCollision3D>  RigidCharacterBody3D::_get_slide_collision(int p_bounce) {
...
// Ref<KinematicCollision3D>  RigidCharacterBody3D::_get_last_slide_collision() {
...
bool  RigidCharacterBody3D::is_floor_stop_on_slope_enabled() const {
	return floor_stop_on_slope;
}

void  RigidCharacterBody3D::set_floor_stop_on_slope_enabled(bool p_enabled) {
	floor_stop_on_slope = p_enabled;
}

bool  RigidCharacterBody3D::is_floor_constant_speed_enabled() const {
	return floor_constant_speed;
}

void  RigidCharacterBody3D::set_floor_constant_speed_enabled(bool p_enabled) {
	floor_constant_speed = p_enabled;
}

bool  RigidCharacterBody3D::is_floor_block_on_wall_enabled() const {
	return floor_block_on_wall;
}

void  RigidCharacterBody3D::set_floor_block_on_wall_enabled(bool p_enabled) {
	floor_block_on_wall = p_enabled;
}

bool  RigidCharacterBody3D::is_slide_on_ceiling_enabled() const {
	return slide_on_ceiling;
}

void  RigidCharacterBody3D::set_slide_on_ceiling_enabled(bool p_enabled) {
	slide_on_ceiling = p_enabled;
}

uint32_t  RigidCharacterBody3D::get_platform_floor_layers() const {
	return platform_floor_layers;
}

void  RigidCharacterBody3D::set_platform_floor_layers(uint32_t p_exclude_layers) {
	platform_floor_layers = p_exclude_layers;
}

uint32_t  RigidCharacterBody3D::get_platform_wall_layers() const {
	return platform_wall_layers;
}

void  RigidCharacterBody3D::set_platform_wall_layers(uint32_t p_exclude_layers) {
	platform_wall_layers = p_exclude_layers;
}

void  RigidCharacterBody3D::set_motion_mode(MotionMode p_mode) {
	motion_mode = p_mode;
}

RigidCharacterBody3D::MotionMode  RigidCharacterBody3D::get_motion_mode() const {
	return motion_mode;
}

void  RigidCharacterBody3D::set_platform_on_leave(PlatformOnLeave p_on_leave_apply_velocity) {
	platform_on_leave = p_on_leave_apply_velocity;
}

RigidCharacterBody3D::PlatformOnLeave  RigidCharacterBody3D::get_platform_on_leave() const {
	return platform_on_leave;
}

int  RigidCharacterBody3D::get_max_slides() const {
	return max_slides;
}

void  RigidCharacterBody3D::set_max_slides(int p_max_slides) {
	ERR_FAIL_COND(p_max_slides < 1);
	max_slides = p_max_slides;
}

real_t  RigidCharacterBody3D::get_floor_max_angle() const {
	return floor_max_angle;
}

void  RigidCharacterBody3D::set_floor_max_angle(real_t p_radians) {
	floor_max_angle = p_radians;
}

real_t  RigidCharacterBody3D::get_floor_snap_length() {
	return floor_snap_length;
}

void  RigidCharacterBody3D::set_floor_snap_length(real_t p_floor_snap_length) {
	ERR_FAIL_COND(p_floor_snap_length < 0);
	floor_snap_length = p_floor_snap_length;
}

real_t  RigidCharacterBody3D::get_wall_min_slide_angle() const {
	return wall_min_slide_angle;
}

void  RigidCharacterBody3D::set_wall_min_slide_angle(real_t p_radians) {
	wall_min_slide_angle = p_radians;
}

const Vector3 &RigidCharacterBody3D::get_up_direction() const {
	return up_direction;
}

void  RigidCharacterBody3D::set_up_direction(const Vector3 &p_up_direction) {
...
void  RigidCharacterBody3D::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("check_on_floor"), & RigidCharacterBody3D::check_on_floor);
	ClassDB::bind_method(D_METHOD("apply_floor_snap"), & RigidCharacterBody3D::apply_floor_snap);

	ClassDB::bind_method(D_METHOD("set_velocity", "velocity"), & RigidCharacterBody3D::set_velocity);
	ClassDB::bind_method(D_METHOD("get_velocity"), & RigidCharacterBody3D::get_velocity);

	ClassDB::bind_method(D_METHOD("set_safe_margin", "margin"), & RigidCharacterBody3D::set_safe_margin);
	ClassDB::bind_method(D_METHOD("get_safe_margin"), & RigidCharacterBody3D::get_safe_margin);
	ClassDB::bind_method(D_METHOD("is_floor_stop_on_slope_enabled"), & RigidCharacterBody3D::is_floor_stop_on_slope_enabled);
	ClassDB::bind_method(D_METHOD("set_floor_stop_on_slope_enabled", "enabled"), & RigidCharacterBody3D::set_floor_stop_on_slope_enabled);
	ClassDB::bind_method(D_METHOD("set_floor_constant_speed_enabled", "enabled"), & RigidCharacterBody3D::set_floor_constant_speed_enabled);
	ClassDB::bind_method(D_METHOD("is_floor_constant_speed_enabled"), & RigidCharacterBody3D::is_floor_constant_speed_enabled);
	ClassDB::bind_method(D_METHOD("set_floor_block_on_wall_enabled", "enabled"), & RigidCharacterBody3D::set_floor_block_on_wall_enabled);
	ClassDB::bind_method(D_METHOD("is_floor_block_on_wall_enabled"), & RigidCharacterBody3D::is_floor_block_on_wall_enabled);
	ClassDB::bind_method(D_METHOD("set_slide_on_ceiling_enabled", "enabled"), & RigidCharacterBody3D::set_slide_on_ceiling_enabled);
	ClassDB::bind_method(D_METHOD("is_slide_on_ceiling_enabled"), & RigidCharacterBody3D::is_slide_on_ceiling_enabled);

	ClassDB::bind_method(D_METHOD("set_platform_floor_layers", "exclude_layer"), & RigidCharacterBody3D::set_platform_floor_layers);
	ClassDB::bind_method(D_METHOD("get_platform_floor_layers"), & RigidCharacterBody3D::get_platform_floor_layers);
	ClassDB::bind_method(D_METHOD("set_platform_wall_layers", "exclude_layer"), & RigidCharacterBody3D::set_platform_wall_layers);
	ClassDB::bind_method(D_METHOD("get_platform_wall_layers"), & RigidCharacterBody3D::get_platform_wall_layers);

	ClassDB::bind_method(D_METHOD("get_max_slides"), & RigidCharacterBody3D::get_max_slides);
	ClassDB::bind_method(D_METHOD("set_max_slides", "max_slides"), & RigidCharacterBody3D::set_max_slides);
	ClassDB::bind_method(D_METHOD("get_floor_max_angle"), & RigidCharacterBody3D::get_floor_max_angle);
	ClassDB::bind_method(D_METHOD("set_floor_max_angle", "radians"), & RigidCharacterBody3D::set_floor_max_angle);
	ClassDB::bind_method(D_METHOD("get_floor_snap_length"), & RigidCharacterBody3D::get_floor_snap_length);
	ClassDB::bind_method(D_METHOD("set_floor_snap_length", "floor_snap_length"), & RigidCharacterBody3D::set_floor_snap_length);
	ClassDB::bind_method(D_METHOD("get_wall_min_slide_angle"), & RigidCharacterBody3D::get_wall_min_slide_angle);
	ClassDB::bind_method(D_METHOD("set_wall_min_slide_angle", "radians"), & RigidCharacterBody3D::set_wall_min_slide_angle);
	ClassDB::bind_method(D_METHOD("get_up_direction"), & RigidCharacterBody3D::get_up_direction);
	ClassDB::bind_method(D_METHOD("set_up_direction", "up_direction"), & RigidCharacterBody3D::set_up_direction);
	ClassDB::bind_method(D_METHOD("set_motion_mode", "mode"), & RigidCharacterBody3D::set_motion_mode);
	ClassDB::bind_method(D_METHOD("get_motion_mode"), & RigidCharacterBody3D::get_motion_mode);
	ClassDB::bind_method(D_METHOD("set_platform_on_leave", "on_leave_apply_velocity"), & RigidCharacterBody3D::set_platform_on_leave);
	ClassDB::bind_method(D_METHOD("get_platform_on_leave"), & RigidCharacterBody3D::get_platform_on_leave);
	...
	ClassDB::bind_method(D_METHOD("get_floor_normal"), & RigidCharacterBody3D::get_floor_normal);
	ClassDB::bind_method(D_METHOD("get_wall_normal"), & RigidCharacterBody3D::get_wall_normal);
	ClassDB::bind_method(D_METHOD("get_last_motion"), & RigidCharacterBody3D::get_last_motion);
	ClassDB::bind_method(D_METHOD("get_position_delta"), & RigidCharacterBody3D::get_position_delta);
	ClassDB::bind_method(D_METHOD("get_real_velocity"), & RigidCharacterBody3D::get_real_velocity);
	ClassDB::bind_method(D_METHOD("get_floor_angle", "up_direction"), & RigidCharacterBody3D::get_floor_angle, DEFVAL(Vector3(0.0, 1.0, 0.0)));
	ClassDB::bind_method(D_METHOD("get_platform_velocity"), & RigidCharacterBody3D::get_platform_velocity);
	ClassDB::bind_method(D_METHOD("get_platform_angular_velocity"), & RigidCharacterBody3D::get_platform_angular_velocity);
	ClassDB::bind_method(D_METHOD("get_slide_collision_count"), & RigidCharacterBody3D::get_slide_collision_count);
	// ClassDB::bind_method(D_METHOD("get_slide_collision", "slide_idx"), & RigidCharacterBody3D::_get_slide_collision);
	// ClassDB::bind_method(D_METHOD("get_last_slide_collision"), & RigidCharacterBody3D::_get_last_slide_collision);
...
void  RigidCharacterBody3D::_validate_property(PropertyInfo &p_property) const {
...
RigidCharacterBody3D::RigidCharacterBody3D() : RigidBody3D()
		// PhysicsBody3D(PhysicsServer3D::BODY_MODE_KINEMATIC) 
{
    
}

RigidCharacterBody3D::~RigidCharacterBody3D() {
	for (int i = 0; i < slide_colliders.size(); i++) {
		if (slide_colliders[i].is_valid()) {
			slide_colliders.write[i]->set_owner(nullptr);
		}
	}
}
